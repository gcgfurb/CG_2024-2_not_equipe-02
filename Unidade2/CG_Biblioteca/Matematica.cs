...
namespace CG_Biblioteca
{
  ...
  public abstract class Matematica
  {
    ...
    public static (Ponto4D pontoFinal, Ponto4D pontoInicial) GerarPontoVetor(Ponto4D pontoInicial, double raio, double angulo)
{
    // Calcula a inclinação em radianos
    double radianos = MathHelper.DegreesToRadians(angulo);
    
    // Calcula as coordenadas do ponto final usando a distância (raio) e o ângulo
    double xFinal = pontoInicial.X + raio * Math.Cos(radianos);
    double yFinal = pontoInicial.Y + raio * Math.Sin(radianos);
    
    // Cria o ponto final
    Ponto4D pontoFinal = new Ponto4D(xFinal, yFinal, pontoInicial.Z, pontoInicial.W);
    
    return (pontoFinal, pontoInicial);
}
